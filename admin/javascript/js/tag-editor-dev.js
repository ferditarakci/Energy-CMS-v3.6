...
;(function ($) {
    $.fn.tagEditor = function (opts) {
        ...
        return this.each(function () {
            ...
            var $$ = $(this).wrap('<span class="tags-container" />'),
            ...
            function addNewTag(tag) {
                if (opts.maxTags <= 0 || allTags.length < opts.maxTags) {
                    if (opts.tagMaxLength <= 0 || tag.length <= opts.tagMaxLength) {
                        var newTag = $('<span class="tag">' + tag + '<a href="#" title="Bu Etiketi Sil">x</a></span>').data('tag', tag);
                        newTag.insertBefore($$).find('a').click(function (e) {
                            removeTag(tag);
							updateHidden(); // Bug Bulundu - 22/06/2012
                            e.preventDefault();
                        });
