...
Sub ArrPageList2( _
	...
	Set arrListPage2(Null) = jsObject()
	arrListPage2(Null)("DT_RowId") = "trid_" & kimlik
	arrListPage2(Null)("DT_RowClass") = ""
	arrListPage2(Null)("counter") = counter
	arrListPage2(Null)("baslik") = baslik
	arrListPage2(Null)("url") = url
	...
	arrListPage2(Null)("durum") = durum
	arrListPage2(Null)("edit") = edit
	arrListPage2(Null)("sil") = sil
End Sub
...
'Search Code
SearchSQL = ""
inputSearch  = Trim(Temizle(Request.Form("sSearch"), 1))
If inputSearch <> "" Then
...
SQL = SQL & "t1.id, t1.sira, t1.ozel, t1.durum, t1.anasyf, /*date_format(t1.c_date, '%d.%m.%Y') As*/ date_format(t5.DateTimes, '%Y/%m/%d') As c_date, t1.s_date, t1.e_date," & vbCrLf
...
'// Ordering
If Request.Form("iSortCol_0") <> "" Then
	SQL = SQL & " "
	Dim aColumns : aColumns = array("", "", "t2.title", "t1.sira", "t3.title", "t5.DateTimes", "t1.id", "t2.hit", "yorum", "", "", "", "")
	
	For i = 0 to intYap(Request.Form("iSortingCols"), 0)
		If Request.Form("bSortable_" & Request.Form("iSortCol_" & i)) = "true" Then
			If i = 0 Then SQL = SQL & "ORDER BY "
			SQL2 = SQL2 & aColumns(intYap(Request.Form("iSortCol_" & i), 0) )
			SQL = SQL & aColumns(intYap(Request.Form("iSortCol_" & i), 0) )
			If Not i = 0 Then SQL = SQL & ", "
			SQL = SQL & " " & Request.Form("sSortDir_" & i) & " "
		End If
	Next
End If
...
'// Paging
if Request.Form("iDisplayStart") <> "" And intYap(Request.Form("iDisplayLength"), 0) > 0 Then
	SQL = SQL & "Limit " & intYap(Request.Form("iDisplayStart"), 0) & ", " & intYap(Request.Form("iDisplayLength"), 0)
End If
...
If Not objRs.Eof Then
	...
	intCounter = intYap(Request.Form("iDisplayStart"), 0)
...
ArrPageList2 _
		intCounter, _
		"<div id="""& objRs("id") &""">" & strTitle & "</div>", _
		UrlWrite("", GlobalConfig("site_lang"), GlobalConfig("General_Page"), "", "", Cdbl(objRs("id")), "", ""), _
		"<input class=""inputbox list-order"" id=""order_"& objRs("id") &""" name=""order[]"" value="""& objRs("sira") &""" maxlength=""4"" autocomplete=""off"" type=""number"" step=""1"" min=""0"" />", _
		, _
		CreateDate, _
		objRs("id"), _
		objRs("hit"), _
		"<a class=""comment-icon"" href=""?mod=yorum&amp;parent="& GlobalConfig("General_PagePN") &"&amp;parent_id="& objRs("id") & sLang & Debug &"""><span>"& objRs("yorum") &"</span></a>", _
		"?mod=url_list&amp;parent="& GlobalConfig("General_PagePN") &"&amp;parent_id="& objRs("id") & sLang & Debug, _
		"<a class="""& StatusStyle(objRs("durum"), "list-passive-icon", "list-active-icon") &" taskListSubmit"" data-number="""& intCounter-1 &""" data-status=""status"" data-apply="""" title=""Aktif/Pasif Yap"">Aktif/Pasif Yap</a>", _
		"<a class=""list-edit-icon"" href=""?mod="& GlobalConfig("General_Page") &"&amp;task=edit&amp;id="& objRs("id") & sLang & Debug &""" title=""Düzenle"">Düzenle</a>", _
		"<a class=""list-delete-icon taskListSubmit"" data-number="""& intCounter-1 &""" data-status=""delete"" data-apply="""" title=""Kalıcı Olarak Sil"">Kalıcı Olarak Sil</a>"
...
'	arrListPage2(Null)("counter") = counter
'	arrListPage2(Null)("checkbox") = checkbox
'	arrListPage2(Null)("baslik") = baslik
'	arrListPage2(Null)("url") = url
'	arrListPage2(Null)("sira") = sira
'	arrListPage2(Null)("tema") = tema
