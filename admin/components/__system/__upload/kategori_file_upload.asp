...
TotalPic = Cdbl(sqlQuery("SELECT Count( id ) FROM #___files WHERE (file_type = 2 And lang = '"& CookieLang &"' And parent = "& GlobalConfig("General_CategoriesPN") &" And parent_id = "& post_id &");", 0))
intSiraNo = Cdbl(sqlQuery("SELECT Max( sira ) FROM #___files WHERE (file_type = 2 And lang = '"& CookieLang &"' And parent = "& GlobalConfig("General_CategoriesPN") &" And parent_id = "& post_id &");", 0))
...
Call CreateFolder(kFolder(post_id, 3))

imgPath = Server.MapPath(kFolder(post_id, 3))
'imgPath2 = Server.MapPath(kFolder(post_id, 1))
...
For Each File In Upload.Files
	...
	Else
		...
		SQL = SQL & "'"& Temizle(ResimAdi, 1) &"', 2, '"& DosyaTuru &"', "& intSiraNo &", "& iDefault &", "& GlobalConfig("General_CategoriesPN") &", "& post_id &", '', '');"
		sqlExeCute( SQL )
		'If data.Errors.Count > 0 Then
		'	Call OpenDatabase(data)
		'	data.ExeCute SQL
		'End If
		files_id = sqlQuery("SELECT id FROM #___files WHERE file_type = 2 And lang = '"& CookieLang &"' And parent = "& GlobalConfig("General_CategoriesPN") &" And parent_id = "& post_id &" ORDER BY id DESC Limit 1;", 0)
