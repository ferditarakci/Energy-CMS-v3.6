<%If Not (mods = "login") Then%>
	<div id="header-box" class="clearfix">
		<div id="module-menu" class="clearfix">
<%
'Clearfix sTotalLang()
...
%>				</li><%If GlobalConfig("urun_yonetimi") Then%>
				...
				If Ubound(splitDomain) = 0 Then
					Response.Write("<a href="""& UrlWrite(GlobalConfig("sDomain"), "", GlobalConfig("General_Sitemap"), "sitemap.xml", "", "", "", "") &""" title=""Site Haritası"" target=""_blank""><span>Site Haritası</span></a>" & vbCrLf)
				Else
					Response.Write("<a href=""javascript:;""><span>Site Haritası</span></a>" & vbCrLf)
					Response.Write("	<ul>" & vbCrLf)
					For Each sDmn In splitDomain
						sDmn = Trim(sDmn)
						Response.Write("		<li><a href="""& UrlWrite("http://" + sDmn, "", GlobalConfig("General_Sitemap"), "sitemap.xml", "", "", "", "") &""" target=""_blank""><span>"&sDmn&"</span></a></li>" & vbCrLf)
					Next
					Response.Write("	</ul>" & vbCrLf)
				End If
				'Dim splitDomain, sDmn
