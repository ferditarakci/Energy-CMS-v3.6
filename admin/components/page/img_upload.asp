...
intDurum = False
If FilesKontrol(sFolder("", -1) & "/" & GlobalConfig("LogoWrite")) Then
	GlobalConfig("LogoWrite") = Server.MapPath(GlobalConfig("hidden_folder") & sFolder("", -1)) & "/" & GlobalConfig("LogoWrite")
	intDurum = True
End If
...
TotalPic = Cdbl(sqlQuery("SELECT Count( id ) FROM #___files WHERE (file_type = 1 And lang = '"& CookieLang &"' And parent = "& GlobalConfig("General_PagePN") &" And parent_id = "& post_id &");", 0))
intSiraNo = Cdbl(sqlQuery("SELECT Max( sira ) FROM #___files WHERE (file_type = 1 And lang = '"& CookieLang &"' And parent = "& GlobalConfig("General_PagePN") &" And parent_id = "& post_id &");", 0))
...
Call CreateFolder(sFolder(post_id, 1))
Call CreateFolder(sFolder(post_id, 2))

imgPath = Server.MapPath(GlobalConfig("hidden_folder") & sFolder(post_id, 0))
imgPath2 = Server.MapPath(GlobalConfig("hidden_folder") & sFolder(post_id, 1))
imgPath3 = Server.MapPath(GlobalConfig("hidden_folder") & sFolder(post_id, 2))
...
For Each File In Upload.Files
	...
	Else
		...
		Else
			...
			SQL = SQL & "'"& Temizle(ResimAdi, 1) &"', 1, '"& DosyaTuru &"', "& intSiraNo &", "& iDefault &", "& GlobalConfig("General_PagePN") &", "& post_id &", '', '');"
			sqlExeCute( SQL )
			'If data.Errors.Count > 0 Then
			'	Call OpenDatabase(data)
			'	data.ExeCute SQL
			'End If
			files_id = sqlQuery("SELECT id FROM #___files WHERE file_type = 1 And lang = '"& CookieLang &"' And parent = "& GlobalConfig("General_PagePN") &" And parent_id = "& post_id &" ORDER BY id DESC Limit 1;", 0)
			...
			Call PictureJson( _
				...
				UploadDurum, _
				addVitrinClass, _
				imgAlign(sFolder(post_id, 1) & "/" & ResimAdi, 110, 90, 110, 90), _
				"", _
