...
'clearfix Server.MapPath(sFolder(57223, 0))
If (pageid > 0) Then Response.Redirect("?mod="& mods &"&task=edit&id="& pageid & Duzenle(sLang & Debug) )

Set objRs = setExecute("SELECT MaxPictureSize, MaxPictureTotalSize, MaxFileSize, MaxFileTotalSize FROM #___settings WHERE opt_status = 1 And parent = "& GlobalConfig("General_PagePN") &";")
...
SQL = SQL & "FROM #___settings WHERE opt_status = 1 And parent = "& GlobalConfig("General_PagePN") &";" & vbCrLf
...
If id > 0 Then
	Set objRs = setExecute("SELECT * FROM #___sayfa WHERE id = "& id &";")
		If objRs.Eof Then
			Response.Write("<div class=""notepad clearfix""><div class=""warning""><div class=""messages""><span>Kayıt bulunamadı. İsterseniz yeni bir kayıt ekleyebilirsiniz.</span></div></div></div>")
		Else
			EditiD = objRs("id")
...
Do While Not objRs.Eof
	...
	'// Content
	SQL = ""
	SQL = SQL & "SELECT t1.id, t1.title, t1.fixed_title, t2.seflink, t1.text, t1.short_text, t1.keyword, t1.description, t1.hit, t1.readmore_text "
	...
	SQL = SQL & "WHERE (t1.lang = '"& strLangCode &"' And t1.parent = "& GlobalConfig("General_PagePN") &" And t1.parent_id = "& EditiD &");"
	'SQL = setQuery( SQL )
	...
	If Not objRs2.Eof Then
		...
		strReadMoreText = HtmlEnCode(objRs2("readmore_text"))
...
strPictures = ""
Set objRs3 = setExecute("SELECT id, IFNULL(title, '') As title, IFNULL(alt, '') As alt, resim, anaresim, featuredFile, durum FROM #___files WHERE (file_type = 1 And parent = "& GlobalConfig("General_PagePN") &" And parent_id = "& EditiD &" And lang = '"& strLangCode &"') ORDER BY sira ASC;")
	Do While Not objRs3.Eof
		...
		strPictures = strPictures & ("<img src="""& sFolder(EditiD, 1) & "/" & objRs3("resim") &""" title="""& objRs3("title") &""" alt="""& objRs3("alt") &""""& imgAlign(sFolder(EditiD, 1) & "/" & objRs3("resim"), 110, 90, 110, 90) & " /></a>" & vbCrLf)
		strPictures = strPictures & ("					</div>" & vbCrLf)
...
strTabContent = strTabContent & "					<li class=""link""><a class=""fancybox fancybox.iframe"" data-fancybox-width=""800"" data-fancybox-height=""400"" tabindex=""-1"" href=""?mod=popup_" & GlobalConfig("General_Page") &"&amp;e_name=text_"& strLangCode &"&amp;"& ewy_queryLang &"=" & LCase(strLangCode) & Debug &""" title=""Sayfa bağlantısı ekle"">Bağlantı Ekle</a></li>" & vbCrLf
...
strTabContent = strTabContent & "				<h3 class=""box-title"" style=""color: #0047FF;"">"& strLangTitle &"İçerik</h3>" & vbCrLf
strTabContent = strTabContent & "			</div>" & vbCrLf
...
strEtiket = ""
Set objRs2 = setExecute("SELECT a.etiket FROM #___etiket As a INNER JOIN #___etiket_id As b ON a.id = b.eid WHERE b.parent = "& GlobalConfig("General_PagePN") &" And b.parent_id = "& EditiD &" And b.lang = '"& strLangCode &"';")
...
strTabContent = strTabContent & "				<div class=""row clearfix"" style=""clear:both; float:left !important;  margin-top:5px; *margin-left:12px;"">" & vbCrLf
...
strTabContent = strTabContent & "					<div class=""clearfix"">" & vbCrLf
...
strMoreText = strMoreText & " <div class=""row clearfix"">" & vbCrLf
strMoreText = strMoreText & "	<div class=""l"" style=""width:52%;""><label for=""readmore_"& strLangCode &"""><span>:</span>"& strLangTitle &"Metin</label></div>" & vbCrLf
strMoreText = strMoreText & "	<div class=""r"" style=""width:45%;"">" & vbCrLf
strMoreText = strMoreText & "		<input style=""width:130px;"" class=""inputbox"" name=""readmore_"& strLangCode &""" id=""readmore_"& strLangCode &""" value="""& strReadMoreText &""" type=""text"" maxlength=""100"" />" & vbCrLf
strMoreText = strMoreText & "	</div>" & vbCrLf
strMoreText = strMoreText & "	<div class=""clr""></div>" & vbCrLf
strMoreText = strMoreText & "</div>" & vbCrLf
...
<div class="energy-tabs">
	<div class="m_box" id="tabs">
		<div class="title"><%=ContentRevizyon(GlobalConfig("General_PagePN"), EditiD) & addTitle%>
			<ul <%=strCssHidden%>>
				<%
...
<div class="m_box">
	<div class="title"><div style="margin-right:5px; margin-top:3px;" id="upload" class="smallicon"><span>Resim Seç</span></div> <h3 class="box-title">Bu Sayfaya Ait Görseller <a class="tooltip" tabindex="-1" href="#" title="">&nbsp;</a></h3></div>
	<div class="head clearfix">
...
'strAppendPictures = "<h3 class=""h3Title"">Öne Çıkarılmış Görseller</h3>"
'strAppendPictures = strAppendPictures & "<div class=""borderBottom clearfix"">" & vbCrLf
'strAppendPictures = strAppendPictures & "	<ul class=""image_files clearfix"" id=""files_o"" data_lang=""TR"">" & vbCrLf
'strAppendPictures = strAppendPictures & strPictures & vbCrLf
'strAppendPictures = strAppendPictures & "	</ul>" & vbCrLf
'strAppendPictures = strAppendPictures & "</div>" & vbCrLf
'Response.Write( strAppendPictures )
			...
			<ul class="clearfix">
				...
				<li><span class="text blue">İsterseniz editörün üst kısmında bulunan &ldquo;<span class="red">Görsel Ekle</span>&rdquo; butonuna tıklayarak &ldquo;<span class="red"><%=GlobalConfig("PageUploadFolder")%></span>&rdquo; klasörü içindeki &ldquo;<span style="font-weight:bold;" class="green"><%=Left(GlobalConfig("PageUploadFolder"), 1)%>_<span id="pgid" class="red"><%If EditiD > 0 Then Response.Write(EditiD) Else Response.Write("{kimlik}")%></span></span>&rdquo; isimli klasörden ilgili dosyayı seçin.</span></li>
		...
		<div class="m_box clearfix">
			...
			<div class="head clearfix">
				<div class="form-table clearfix">
					...
					<div id="home_page_alias_bar" class="row clearfix<%If Not EditAnasyf Then Response.Write(" hidden")%> clearfix">
...
SQL = SQL & "WHERE durum = 1 And (parent = "& GlobalConfig("General_PagePN") &" Or parent = 0) "
...
Set objRs = Nothing
		...
		<div class="m_box">
			<div class="title"><h3 class="box-title">Sayfa Özellikleri</h3></div>
			<div class="head clearfix">
				<div class="form-table clearfix">
					<div class="row clearfix">
					...
					<div class="row clearfix">
						...
						<div class="r">
							...
							<a class="tooltip" tabindex="-1" href="#" title="Sayfalar genellikle alfabetik olarak sıralanır, <br />fakat isterseniz bu alana sayı girerek (birinci için 1 ) <br />şeklinde kendiniz de belirleyebilirsiniz.">&nbsp;</a>
						</div>
						<div class="clr"></div>
					</div>
					<div class="row clearfix">
					...
					<div class="row clearfix">
						<div style="margin-bottom: 5px;"><label for="anaid">Ebeveyn
							<a class="tooltip" tabindex="-1" href="#" title="Sayfalarınızı hiyerarşik olarak düzenleyebilirsiniz. <br />Örneğin &ldquo;Devam eden projeler&rdquo; ve &ldquo;Biten Projeler&rdquo; şeklinde iki sayfaya sahip bir &ldquo;Referanslar&rdquo; sayfanız olabilir.<br /> Alt alta kaç seviye oluşturacağınız konusunda limitsizsiniz.">&nbsp;</a>
						</label></div>
						<div>
							<select style="width:100%;" name="anaid" id="anaid" size="1">
								<option value="0"<%=eSelected(EditAnaiD = 0)%>>Üst</option>
									<%Call EbeveynSayfa(0, 0, EditAnaiD)%>
							</select>
						</div>
						<div class="clr"></div>
					</div>
...
SQL = SQL & "WHERE durum = 1 And (parent = "& GlobalConfig("General_PagePN") &" Or parent = 0) ORDER BY sira ASC;"
'SQL = setQuery( SQL )
Set objRs = setExecute( SQL )
	While Not objRs.Eof
		strSelected = ""
...
SQL = SQL & "IF((SELECT id FROM #___content_menu WHERE (menu_id = t1.id And parent = "& GlobalConfig("General_PagePN") &" And parent_id = "& EditiD &") Limit 1) Is Null, '', ' selected=""selected""') As strSelected" & vbCrLf
...
strFiles = ""
Set objRs = setExecute("SELECT id, resim, IFNULL(title, '') As title FROM #___files WHERE (file_type = 2 And parent = "& GlobalConfig("General_PagePN") &" And parent_id = "& EditiD &") ORDER BY sira ASC;")
	...
	Do While Not objRs.Eof
		...
		strFiles = strFiles & ("					<a class=""other_file_edit"" tabindex=""-1"" href="""& sFolder(EditiD, 3) & "/" & objRs("resim") &""" title="""& objRs("title") &""">" & KacKarekter(objRs("resim"), 47) & "</a>")
...
%>
		...
		<div class="m_box">
			<div class="title"><h3 class="box-title">Devamını Oku Metni <a class="tooltip" tabindex="-1" href="#" title="Varsayılan devamını oku mentinin yerini alacak bir metin girin.">&nbsp;</a></h3></div>
			<div class="head clearfix">
				<div class="form-table clearfix">
<%
Response.Write( strMoreText )
%>
				</div>
			</div>
		</div>
		...
		<div class="m_box">
			...
			<div class="head clearfix">
				<div class="form-table clearfix">
					...
					<div style="text-align:center;" id="pass-div" class="row clearfix<%If (EditOzelSayfa < 2) Then Response.Write(" hidden")%>">
						<div>
							<label class="label_inline" for="pass">Parola&nbsp;Girin <input style="width:130px" class="inputbox" name="pass" id="pass" value="<%=EditPass%>" maxlength="20" autocomplete="off" type="text" /></label>
							<a class="tooltip" tabindex="-1" href="#" title="Bu özellik sayfanızın belirli kullanıcılar tarıfından <br />görüntülenmesini sağlamak amacıyla oluşturulmuştur, <br />kendi belirleyeceğiniz maksimun 16 karekter uzunluğunda <br />bir parolayı girdiğiniz taktirde sayfa görüntülenecektir.">&nbsp;</a>
						</div>
						<div class="clr"></div>
					</div>
				</div>
