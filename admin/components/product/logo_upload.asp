...
Call CreateFolder(pFolder("", -1))

imgPath = Server.MapPath(GlobalConfig("hidden_folder") & pFolder("", -1))
...
For Each File In Upload.Files
	...
	Else
		...
		Else
			...
			Call DeleteFile( pFolder("", -1) & "/" & sqlQuery("SELECT LogoWrite FROM #___settings WHERE parent = "& GlobalConfig("General_ProductsPN") &";", "") )

			'// Veri tabanına resim bilgilerini girelim
			sqlExeCute("UPDATE #___settings Set LogoWrite = '"& ResimAdi &"' WHERE parent = "& GlobalConfig("General_ProductsPN") &";")
