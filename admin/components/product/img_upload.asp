...
intDurum = False
If FilesKontrol(pFolder("", -1) & "/" & GlobalConfig("LogoWrite")) Then
	GlobalConfig("LogoWrite") = Server.MapPath(GlobalConfig("hidden_folder") & pFolder("", -1)) & "/" & GlobalConfig("LogoWrite")
	intDurum = True
End If

OpenRs objRs, "SELECT id, durum FROM #___products WHERE id = "& id &";"
	If objRs.Eof Then
		objRs.AddNew
		objRs("durum") = 0
		'objRs("c_date") = DateSqlFormat(Now(), "yy-mm-dd", 1)
		objRs.Update
		'data.ExeCute(setQuery("INSERT INTO #___content (lang, parent, parent_id, title, short_title, text, short_text) VALUES ('"& CookieLang &"', '"& GlobalConfig("General_ProductsPN") &"', "& objRs("id") &", '( Başlık yok )', '', '', '');"))
		Call RevisionDate(GlobalConfig("General_ProductsPN"), objRs("id"), 1) '// 1 Insert -- 2 Revision
		sqlExeCute("INSERT INTO #___content (lang, parent, parent_id, title) VALUES ('"& CookieLang &"', "& GlobalConfig("General_ProductsPN") &", "& objRs("id") &", '( Başlık yok )');")
	End If
	'objRs.Requery
	post_id = objRs("id")
...
TotalPic = Cdbl(sqlQuery("SELECT Count( id ) FROM #___files WHERE (file_type = 1 And lang = '"& CookieLang &"' And parent = "& GlobalConfig("General_ProductsPN") &" And parent_id = "& post_id &");", 0))
intSiraNo = Cdbl(sqlQuery("SELECT Max( sira ) FROM #___files WHERE (file_type = 1 And lang = '"& CookieLang &"' And parent = "& GlobalConfig("General_ProductsPN") &" And parent_id = "& post_id &");", 0))

sUpFolder = GlobalConfig("uploadFolder") & "/" & GlobalConfig("ProductsUploadFolder")
If Not FolderKontrol( sUpFolder ) Then Call CreateFolder( sUpFolder )
sUpFolder = ""

'// fotoğrafların yükleneceği klasörümüzü oluşturuyoruz
Call CreateFolder(pFolder(post_id, 0))
Call CreateFolder(pFolder(post_id, 1))
Call CreateFolder(pFolder(post_id, 2))

imgPath = Server.MapPath(GlobalConfig("hidden_folder") & pFolder(post_id, 0))
imgPath2 = Server.MapPath(GlobalConfig("hidden_folder") & pFolder(post_id, 1))
imgPath3 = Server.MapPath(GlobalConfig("hidden_folder") & pFolder(post_id, 2))
...
For Each File In Upload.Files
	...
	Else
		...
		Else
			...
			'	objRs("nerde") = GlobalConfig("General_ProductsPN")
			'	objRs.Update
			'	files_id = objRs("id")
			'CloseRs objRs
			...
			SQL = SQL & "'"& Temizle(ResimAdi, 1) &"', 1, '"& DosyaTuru &"', "& intSiraNo &", "& iDefault &", "& GlobalConfig("General_ProductsPN") &", "& post_id &", '', '');"
			sqlExeCute( SQL )
			'If data.Errors.Count > 0 Then
			'	Call OpenDatabase(data)
			'	data.ExeCute SQL
			'End If
			files_id = sqlQuery("SELECT id FROM #___files WHERE file_type = 1 And lang = '"& CookieLang &"' And parent = "& GlobalConfig("General_ProductsPN") &" And parent_id = "& post_id &" ORDER BY id DESC Limit 1;", 0)
			...
			Call PictureJson( _
				...
				UploadDurum, _
				addVitrinClass, _
				imgAlign(pFolder(post_id, 1) & "/" & ResimAdi, 110, 90, 110, 90), _
				"", _
