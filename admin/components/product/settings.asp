...
<body>
<%
Set objRs = setExecute("SELECT * FROM #___settings WHERE opt_status = 1 And parent = "& GlobalConfig("General_ProductsPN") &";")
	If Not objRs.Eof Then
		...
		GlobalConfig("LogoWrite") = "" ' "<img src=""" & pFolder("", -1) & "/" & objRs("LogoWrite") & """ />"
		...
		If objRs("LogoWrite") <> "" Then
			...
			GlobalConfig("LogoWrite") = GlobalConfig("LogoWrite") & "<img src=""" & pFolder("", -1) & "/" & objRs("LogoWrite") & """ />"
...
SQL = SQL & "LEFT JOIN #___settings_lang As b ON a.lng = b.lang And b.parent = "& GlobalConfig("General_ProductsPN") &"" & vbCrLf
...
<input name="parent" type="hidden" value="<%=GlobalConfig("General_ProductsPN")%>" />
