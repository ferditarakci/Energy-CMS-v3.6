...
Call CreateFolder(kFolder("", -1))

imgPath = Server.MapPath(GlobalConfig("hidden_folder") & kFolder("", -1))
...
For Each File In Upload.Files
	...
	Else
		...
		Else
			...
			Call DeleteFile( kFolder("", -1) & "/" & sqlQuery("SELECT LogoWrite FROM #___settings WHERE parent = "& GlobalConfig("General_CategoriesPN") &";", "") )

			'// Veri tabanına resim bilgilerini girelim
			sqlExeCute("UPDATE #___settings Set LogoWrite = '"& ResimAdi &"' WHERE parent = "& GlobalConfig("General_CategoriesPN") &";")
